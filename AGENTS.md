...
- **`Table.List`**, **`Table.Row`**, **`Table.Separator`** — shared list row layout (home, search, members, admin lists, etc.). Single-line **`Table.Row`** instances are **`h-11`** (44px); rows with **`wrapLabel`** use **`min-h-11`** and grow when the label wraps. **`Table.Row`** takes **`label`**, optional **`href`**, and an optional **`trailing`** snippet (laid out with **`gap-1.5`**) for badges/actions.
